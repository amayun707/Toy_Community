...
<table id="notice">
    ...
    if (count != 0) {
    	for (int i = 0; i < boardList.size(); i++, row++) {
    		if (row % col == 1) {
    %>
    <tr>
    <%
    ...
    %>
        <td><img src="../upload/<%=gb.getFile()%>" width="100" height="100" onclick="location.href='g_content.jsp?num=<%=gb.getNum()%>&pageNum=<%=pageNum%>'"> 
        <br> <label>제목 : </label><%=gb.getSubject()%><br> <label>작성자 : </label><%=gb.getName()%><br> 
        <label>작성일 : </label><%=sdf.format(gb.getDate())%></td>
        <%
            if (row % col == 0) {
        %>
        </tr>
