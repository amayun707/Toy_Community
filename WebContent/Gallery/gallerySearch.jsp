...
List boardList = gdao.getBoardList(startRow, pageSize, search);
...
<tr><th class="tno">No.</th>
    ...
    if(count!=0){
        ...
        %>
            <tr onclick="location.href='d_content.jsp?num=<%=gb.getNum()%>&pageNum=<%=pageNum%>'"><td><%=num-- %></td>
                <td class="left"><%=gb.getSubject() %></td>
                <td><%=gb.getName() %></td>
                <td><%=sdf.format(gb.getDate()) %></td>
                <td><%=gb.getReadcount() %></td></tr>
        <%
        }
    }
    %>
...
if(id != null){
    ...
    onclick="location.href='g_fwriteForm.jsp'">
    </div>
    <%
}
%>
...
if(startPage > pageBlock){
    %><a href="gallerySearch.jsp?pageNum=<%=startPage-pageBlock%>&search=<%=search %>">[이전] </a><%
}

//1 2 3 ... 10      11 12 13 ... 20
for(int i=startPage;i<=endPage;i++){
    %><a href="gallerySearch.jsp?pageNum=<%=i%>&search=<%=search %>"><%=" " + i%></a><%
}

//[다음]
if(endPage < pageCount){
    %><a href="gallerySearch.jsp?pageNum=<%=startPage+pageBlock%>&search=<%=search %>"> [다음]</a><%
}
%>
</div>
</article>
